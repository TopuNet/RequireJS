...
function extHtml(content, callback, file) {
    var reg = /((qll-img|qll-bg)=\s?"[\s\S]*?")|(<script(?:(?=\s)[\s\S]*?["'\s\w\/\-]>|>))([\s\S]*?)(?=<\/script\s*>|$)|(<style(?:(?=\s)[\s\S]*?["'\s\w\/\-]>|>))([\s\S]*?)(?=<\/style\s*>|$)|<(img|embed|audio|video|link|object|source)\s+[\s\S]*?["'\s\w\/\-](?:>|$)|<!--inline\[([^\]]+)\]-->|<!--(?!\[)([\s\S]*?)(-->|$)/ig;

    callback = callback || function(m, $9, $10, $1, $2, $3, $4, $5, $6, $7, $8) {
        ...
        if ($1) { //<script>
            ...
            if (embed) {
                ...
                // console.log("rType:" + rType.test($1) ? 'html' : 'js');
